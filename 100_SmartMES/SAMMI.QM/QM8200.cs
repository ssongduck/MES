...
namespace SAMMI.QM
{
    public partial class QM8200 : SAMMI.Windows.Forms.BaseMDIChildForm
    {
        ...
        public override void DoInquire()
        {
            ...
            try
            {
                ...
                #region [검사항목 조회]
                string sPlantCode = SqlDBHelper.nvlString(cboPlantCode_H.Value);                                                                        // 사업장(공장)
                ...
                MonthDiff = Convert.ToInt32(DateAndTime.DateDiff(DateInterval.Month, frRegDT, toRegDT));
                ...
                rtnDtTemp2 = helper.FillTable("USP_QM8200_S2N_UNION", CommandType.StoredProcedure, param2);
                ...
                {
                    this.ClosePrgForm();
                    SAMMI.Windows.Forms.CheckForm checkform = new Windows.Forms.CheckForm("공정 능력조회를 실패하였습니다.");
                    checkform.ShowDialog();
                    return;
                }
                #endregion
        ...
        private void Grid_Data(DataTable dt, string CP)
        {
            ...
            {
                this.DtTable.Columns.Add(new DataColumn(frRegDT.ToString("yyyy-MM"), typeof(string)));
            ...
            try
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (rowid >= 0)
                    {
                        string colname = dt.Rows[i]["YYYYMM"].ToString();
                        colname = colname.Insert(4, "-");
                ...
                for (int j = 0; j < DtTable.Rows.Count; j++)
                {
                    ...
                    {
                        if (Convert.ToDouble(DtTable.Rows[j][i]) > 0)
                        {
                            iCnt += 1;
                        }
                    ...
                    DtTable.Rows[j]["평균"] = (iCnt == 0) ? 0 : Math.Round(cp / iCnt, 3);
            ...
            for (int i = 0; i < this.DtTable.Columns.Count; i++)
            {
                ...
                else
                {
                    ...
                    this.grid2.DisplayLayout.Bands[0].Columns[i].CellAppearance.TextHAlign = Infragistics.Win.HAlign.Left;
                }
            }
            //this.grid2.DisplayLayout.Bands[0].Columns[1].Hidden = true;
            //this.grid2.DisplayLayout.Bands[0].Columns[1].Hidden = true;
            // this.grid2.DisplayLayout.Bands[0].Columns[0].Hidden = true;
        }
        #endregion
        ...
        private void setChart()
        {
            foreach (KeyValuePair<string, NumericSeries> kvp in SeriesDict)
            {
                ...
                for (int i = 0; i < Convert.ToInt32(this.MonthDiff.ToString()) + 1; i++)
                {
                    bool isinsert = false;
                    for (int j = 0; j < this.DtChange2.Rows.Count; j++)
                    {
                        //Convert.ToDateTime(dt.Rows[i]["WorkDT"]).ToString("YYYYMM");
                        if (frRegDT.ToString("yyyy-MM") == this.DtChange2.Rows[j]["YYYYMM"].ToString().Insert(4, "-"))
                        {
                    ...
                    if (!isinsert)
                        SeriesDict[key].Points.Add(new NumericDataPoint(0, frRegDT.ToString("MM"), false));
                    frRegDT = frRegDT.AddMonths(1);
                }
            }
        }
        ...
        private void GridInit1()
        {
            ...
            _GridUtil.InitColumnUltraGrid(grid1, "ItemName", "품목명", false, GridColDataType_emu.VarChar, 150, 100, Infragistics.Win.HAlign.Left, false, false, null, null, null, null, null);
        ...
        private void QM8200_Load(object sender, EventArgs e)
        {
