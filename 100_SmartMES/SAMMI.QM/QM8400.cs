...
namespace SAMMI.QM
{
    public partial class QM8400 : SAMMI.Windows.Forms.BaseMDIChildForm
    {
        #region <MEMBER AREA>
        ...
        public override void DoInquire()
        {
            ...
            MonthDiff = Convert.ToInt32(DateAndTime.DateDiff(DateInterval.Month, frRegDT, toRegDT)) + 1;
            try
            {
                ...
                rtnDtTemp = helper.FillTable("USP_QM8400_S2_UNION", CommandType.StoredProcedure, param1);
            }
            catch (Exception ex)
            {
        ...
        private void GetSeries()
        {
            ...
            this.RLCL = Convert.ToDouble(this.rtnDtTemp.Rows[0]["RR_CL"]);
            ...
            foreach (KeyValuePair<string, NumericSeries> kvp in SeriesDict)
            {
                ...
                for (int i = 0; i < this.MonthDiff; i++)
                {
                    string key = kvp.Key;
                    if (key.Substring(0, 2) == "XX" && key.ToString() != "XX_X")
                    {
                        ...
                        if (Convert.ToDouble(this.DtTable.Rows[0][key]) == 0)
                            SeriesDict[this.DtTable.Columns[key].ColumnName.ToString()].Points
                                               .Add(new NumericDataPoint(Convert.ToDouble(this.DtTable.Rows[0][key])
                                                                        , frRegDT.ToString("yyyy-MM"), true));
                        else
                            SeriesDict[this.DtTable.Columns[key].ColumnName.ToString()].Points
                                               .Add(new NumericDataPoint(Convert.ToDouble(this.DtTable.Rows[0][key])
                                                                        , frRegDT.ToString("yyyy-MM"), false));
            ...
            foreach (KeyValuePair<string, NumericSeries> kvp in SeriesDict)
            {
                ...
                for (int i = 0; i < this.MonthDiff; i++)
                {
                    for (int j = 0; j < this.DtTable.Rows.Count; j++)
                    {
                        string key = kvp.Key;
                        if (key == "XX_X")
                        {
                            if (this.DtTable.Rows[j]["WorkDT"].ToString() == frRegDT.ToString("yyyy-MM").Replace("-", "/"))
                            {
                                this.xbar_lineLayer.Series.Add(SeriesDict[this.DtTable.Columns[key].ColumnName.ToString()]);
                                if (Convert.ToDouble(this.DtTable.Rows[j][key]) == 0)
                                    SeriesDict[this.DtTable.Columns[key].ColumnName.ToString()].Points
                                                       .Add(new NumericDataPoint(Convert.ToDouble(this.DtTable.Rows[j][key])
                                                                                , frRegDT.ToString("yyyy-MM"), true));
                                else
                                    SeriesDict[this.DtTable.Columns[key].ColumnName.ToString()].Points
                                                       .Add(new NumericDataPoint(Convert.ToDouble(this.DtTable.Rows[j][key])
                                ...
                                SeriesDict[this.DtTable.Columns[key].ColumnName.ToString()].Points
                                                   .Add(new NumericDataPoint(0, frRegDT.ToString("yyyy-MM"), false));
                        }
                        if (j == this.DtTable.Rows.Count - 1)
                        {
                            if (key == "RR_R")
                            {
                                if (this.DtTable.Rows[j]["WorkDT"].ToString() == frRegDT.ToString("yyyy-MM").Replace("-", "/"))
        ...
        private void GetGridData(DataTable dt)
        {
            ...
            DateTime frRegDT = Convert.ToDateTime(((DateTime)this.cboStartDate_H.Value).ToString("yyyy-MM-dd") + " 00:00:00.00");
            DateTime toRegDT = Convert.ToDateTime(((DateTime)this.cboEndDate_H.Value).ToString("yyyy-MM-dd") + " 23:59:59.99");

            // 날짜 빼기
            int monthdiff = Convert.ToInt32(DateAndTime.DateDiff(DateInterval.Month, frRegDT, toRegDT));
            for (int i = 0; i < monthdiff + 1; i++)
            {
                this.GridTable.Columns.Add(new DataColumn(frRegDT.ToString("yyyy-MM"), typeof(string)));
                frRegDT = frRegDT.AddMonths(1);
            }
            ...
            {
                this.GridTable.Rows[0][Convert.ToDateTime(dt.Rows[i]["WorkDT"]).ToString("yyyy-MM")] = Math.Round(Convert.ToDouble(dt.Rows[i]["XX_X"]), 3);
                this.GridTable.Rows[1][Convert.ToDateTime(dt.Rows[i]["WorkDT"]).ToString("yyyy-MM")] = Math.Round(Convert.ToDouble(dt.Rows[i]["RR_R"]), 3);
            }
        ...
        #region [폼 로드]
        private void QM8400_Load(object sender, EventArgs e)
        {
            this.cboStartDate_H.Value = Convert.ToDateTime(System.DateTime.Now.ToString("yyyy-01-01 00:00:00"));
        }
        #endregion
        ...
        #region [트리 데이터 생성]
        private void treItem_InitializeDataNode_1(object sender, InitializeDataNodeEventArgs e)
        {
            if (e.Node.Parent == null && e.Node.Cells["ParentKey"].Value.ToString() != "$")
            {
                e.Node.Visible = false;
        ...
        private void XbarChart_InterpolateValues(object sender, Infragistics.UltraChart.Shared.Events.InterpolateValuesEventArgs e)
        {
